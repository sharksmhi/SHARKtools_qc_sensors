...
import gui.widgets as main_gui_widgets
...
#         self.reset_widget()
    ...
    def _callback_time_widget_to(self):
        ...
        try:
            ...
            main_gui_widgets.show_warning(e.message)
...
#        self._set_limits_in_plot_object()
    ...
    def _callback(self):
        ...
        if self.callback:
            ...
            logger.info('No callback function given to AxisSettingsWidget')
...
class CompareWidget(tk.Frame):
    def __init__(self, 
        ...
        if self.include_depth:
            self.set_data(time=self.parent_app.user.match.setdefault('hours', '24'),
                          dist=self.parent_app.user.match.setdefault('dist', '5000'),
                          depth=self.parent_app.user.match.setdefault('depth', '2'))
        else:
            self.set_data(time=self.parent_app.user.match.setdefault('hours', '24'),
                          dist=self.parent_app.user.match.setdefault('dist', '5000'))
    ...
    def update_parameter_list(self, file_id):
        ...
        default_parameter = self.parent_app.user.parameter_priority.get_priority(parameter_list)
    ...
    def _save(self, entry=None):
        if self.include_depth:
            ...
            self.parent_app.user.match.set('hours', str(self.time))
            self.parent_app.user.match.set('dist', str(self.dist))
            self.parent_app.user.match.set('depth', str(self.depth))
        else:
            try:
                self.time = float(self.entry['time'].get_value())
                self.dist = float(self.entry['dist'].get_value())
        ...
        # Save parameter
        self.parent_app.user.parameter_priority.set_priority(self.get_parameter())
...
class FilterPopup(object):
    ...
    def __init__(self,
        ...
        self.session = session
        self.user = self.parent_app.user
    ...
    def display(self):
        ...
        root_position = self.main_app.get_root_window_position()
    ...
    def _reset_filter(self):
        if self.filter_popup:
            self.parent_app.user.filter.reset()
...
class FilterWidgetTable(tk.Frame):
    ...
    def _filter_data(self):
        ...
        if not self.session.get_filtered_file_id_list(startswith=self.file_id_startswith):
            main_gui_widgets.show_information('No files loaded', 'No files loaded for this sampling type.')
            return
    ...
    def _set_frame(self):
        ...
        for key in sorted(self.options):
            ...
            if key.lower() == 'user':
                ...
                self.widgets[key].set_value(self.parent_app.version)
...
class QCroutineWidgetSingle(tk.Frame):
    """
    Widget to show and select(deselect a qc routine. Widget includes option popup frame.
    """
    def __init__(self,
                 parent,
                 parent_app=None,
    ...
    def _set_frame(self):
        ...
        try:
            self.options_image = tk.PhotoImage(
                file=os.path.join(os.path.dirname(__file__), 'pic', 'options_icon2.gif'))
            self.options = tk.Label(self, image=self.options_image, cursor="hand2")
            self.options.grid(row=0, column=0, **grid)
            self.options.bind("<Button-1>", self._on_click_options)
        except:
            # Does not
            self.options = tk.Button(self, text='Settings', command=self._on_click_options)
            self.options.grid(row=0, column=0, **grid)
